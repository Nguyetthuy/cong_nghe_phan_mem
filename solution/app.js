...
async function createAccount(account) {
  return sendRequest('/auth/register', 'POST', account, false);
}
...
async function register() {
  ...
  const loginResult = await loginUser({ user: data.user, password: data.password });
...
async function confirmTransaction() {
  ...
  const data = await createTransaction(state.account.user, transaction);
  if (data.error) {
